...
def run_k_means_clustering(repaired_data):
    ...
    print(df)
    outlier_start = len(df)*98.0/100
    i = 0
    outlier_data = []
    for index, row in df.iterrows():
        #print(index)
        if(i>outlier_start):
            outlier_data.append(index)
            y_km[index]=10
        i+=1
    print((outlier_data))
...
def print_outliers(selected_data, outliers):
...
outliers = run_k_means_clustering(selected_data)
print_outliers(selected_data, outliers)
